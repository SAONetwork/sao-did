...
// parseQuery is a parserStep that extracts a DID Fragment from a DID Reference
// from the grammar:
//   did-query     = *( pchar / "/" / "?" )
//   pchar         = unreserved / pct-encoded / sub-delims / ":" / "@"
//   unreserved    = ALPHA / DIGIT / "-" / "." / "_" / "~"
...
func (p *parser) parseQuery() parserStep {
	...
	for {
		...
		// did-query = *( pchar / "/" / "?" )
		// pchar = unreserved / pct-encoded / sub-delims / ":" / "@"
		// isNotValidQueryFragmentChar checks for all othe valid chars except pct-encoded
		if !percentEncoded && isNotValidQueryFragmentChar(char) {
...
// isNotValidQueryFragmentChar returns true if a byte is not allowed in a Query or a Fragment
// from the grammar:
//   did-query = *( pchar / "/" / "?" )
//   did-fragment = *( pchar / "/" / "?" )
//   pchar        = unreserved / pct-encoded / sub-delims / ":" / "@"
// pct-encoded is not checked in this function
func isNotValidQueryFragmentChar(char byte) bool {
